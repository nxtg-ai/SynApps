...
**Action Items**:
1. [x] **Structured execution logs** — per-node: input, output, duration, errors, retry attempts. Stored per-run.
2. [x] **`GET /executions/:id/logs`** — fetch full execution trace.
3. [x] **Debug mode** — `?debug=true` on execute endpoint returns logs inline with results.
4. [x] Tests.

**CHAIN**: When done, start DIRECTIVE-NXTG-20260318-109.
**Response** (filled by team): N-25 shipped. `ExecutionLogStore` singleton (thread-safe, in-memory, append-only) emits 5 event types: `node_start`, `node_retry`, `node_success`, `node_fallback`, `node_error` — one per lifecycle transition in `_execute_single_node`. `GET /api/v1/executions/{run_id}/logs` returns `{run_id, count, logs[]}` or 404. `POST /flows/{id}/runs?debug=true` polls until terminal and returns logs inline. 19 tests in `test_execution_logs.py` (8 unit + 3 integration + 4 endpoint + 4 debug-mode). Total: 1,688 backend (+19). OpenAPI regenerated (82 paths). 2026-03-18.
...
**Action Items**:
1. [x] Archive DONE directives. 2. [x] README — all 25 initiatives. 3. [x] Final test count (1,797).

**Response** (filled by team): 15 directives archived to NEXUS-archive.md (D-313-02, D-314-08, D-03, D-51, D-50, D-57, D-69, D-68, D-67, D-78, D-77, D-84, D-93, D-92, D-91). README `## Initiatives` table covers N-01 through N-25. Final test count: 1,797 (1,688 backend + 109 frontend + 4 E2E). CHANGELOG updated with N-25 entry. 2026-03-18.
...
> Last updated: 2026-03-21 (cycle 92) — N-174–N-193 shipped (20 features); 3,978 backend tests; CI green; OpenAPI spec fixed
