...
- **CI fix: OpenAPI spec regenerated** — `docs/openapi.json` was ~40 routes stale (N-89 through N-127 added routes not reflected). Regenerated from live app; CI "Verify OpenAPI spec is current" check now passes.
- **N-128: npm Security Fix** — `dompurify ^3.3.3` override in `package.json` resolves GHSA-v2wj-7wpq-c8vv (moderate XSS in dompurify 3.1.3–3.3.1 via monaco-editor transitive dep). `npm audit` now reports 0 vulnerabilities (was 4: 2 high + 2 moderate).
- **CRUCIBLE audit (cycle 90)**: Gate 2 — added `len >= 1` guards to `test_marketplace_api.py` (cloned nodes) and `test_health_dashboard.py` (providers list). Gate 5 — added `OMIT JUSTIFIED` comments to 2 bare `pass`/`continue` blocks in `main.py`. 7/8 gates PASS; 95% backend coverage.
- **N-127: Marketplace Publish** — `MarketplacePublishPage` at `/marketplace-publish`. Covers `POST /api/v1/marketplace/publish` — the last genuinely uncovered backend endpoint. Form with flow ID, name, category dropdown (notification/data-sync/monitoring/content/devops), comma-separated tags, author. Success result panel shows listing ID, name, category, tags. Error handling for 404 (flow not found) and 409 (duplicate). 15 tests. **Frontend endpoint coverage complete: all 211 `@v1.*` routes now have corresponding frontend pages.** 1,090 frontend tests, 3,787 total.
- **N-125–N-126: Frontend Coverage Sprint VI** — `ExecutionHistoryPage` (`GET /api/v1/history` — paginated filtered list with status/template/date filters, 16 tests; `GET /api/v1/history/{run_id}` — full detail with execution trace table, clicking browse rows auto-opens detail), `PluginSchemaPage` (`GET /api/v1/plugins/{plugin_id}/schema` — properties table with type/required/description/default, enum rendering, raw JSON viewer, 13 tests). 1,075 frontend tests, 3,772 total.
- **N-124: Flaky Test Root Cause Fix** — Added `except asyncio.CancelledError` handler in `_execute_flow_async` success path (between `workflow_run_repo.save(status)` and `broadcast_status_fn`/`emit_event` calls). `asyncio.CancelledError` is a `BaseException` in Python 3.8+, not caught by `except Exception` — when TestClient tears down the event loop after `_poll_run` sees "success", the background task's subsequent `await` calls raised `CancelledError` unhandled. Fix absorbed at the function level with `logger.debug` log. All 5 formerly-flaky E2E integration tests now pass in full-suite runs. 2,697 backend tests pass.
