...
async def test_execute_flow_basic(db):
    flow_data = {
        ...
        "edges": [{"id": "e1", "source": "start-node", "target": "end-node"}],
    }
...
async def test_execute_flow_with_applet(db):
    ...
    with patch(
        "apps.applets.writer.applet.WriterApplet.on_message", new_callable=AsyncMock
    ) as mock_on_message:
...
async def test_execute_flow_error(db):
    flow_data = {
        ...
        "edges": [{"id": "e1", "source": "start", "target": "fail-node"}],
    }

    with patch(
        "apps.applets.writer.applet.WriterApplet.on_message", side_effect=Exception("Applet failed")
    ):
        with patch("apps.orchestrator.main.broadcast_status", new_callable=AsyncMock):
