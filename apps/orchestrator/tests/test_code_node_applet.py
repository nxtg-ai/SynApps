...
def mock_subprocess_exec():
    ...
    mock_proc.stdout.read.side_effect = [
        b"",
        b"",
    ]  # Default to empty, tests will set actual content
...
async def test_code_node_python_execution(code_node_applet, mock_subprocess_exec):
    ...
    message = AppletMessage(
        content={"input": 5}, metadata={"node_data": {"language": "python", "code": code}}
    )
...
async def test_code_node_javascript_execution(code_node_applet, mock_subprocess_exec):
    ...
    message = AppletMessage(
        content={"input": 7}, metadata={"node_data": {"language": "javascript", "code": code}}
    )
...
""",
    ...
    message = AppletMessage(
        content={}, metadata={"node_data": {"language": "python", "code": code}}
    )
...
""",
    ...
    message = AppletMessage(
        content={}, metadata={"node_data": {"language": "javascript", "code": code}}
    )
...
""",
    ...
    message = AppletMessage(
        content={},
        metadata={
            "node_data": {"language": "python", "code": code, "env": {"TEST_VAR": "TEST_VAR_VALUE"}}
        },
    )
...
@pytest.mark.asyncio
async def test_code_node_environment_sanitization_javascript(
    code_node_applet, mock_subprocess_exec
):
    ...
    message = AppletMessage(
        ...
        metadata={
            "node_data": {
                "language": "javascript",
                "code": code,
                "env": {"JS_TEST_VAR": "JS_TEST_VAR_VALUE"},
            }
        },
    )
...
async def test_code_node_with_custom_working_dir(code_node_applet, mock_subprocess_exec):
    ...
    message = AppletMessage(
        content={},
        metadata={
            "node_data": {"language": "python", "code": code, "working_dir": "/tmp/my_custom_dir"}
        },
    )
...
async def test_code_node_empty_code_text(code_node_applet):
    ...
    message = AppletMessage(content={}, metadata={"node_data": {"language": "python", "code": ""}})
    response = await code_node_applet.on_message(message)
    assert response.metadata["status"] == "error"
    assert "No code provided" in response.content["error"]

    message_no_code_in_metadata = AppletMessage(
        content={"code": ""}, metadata={"node_data": {"language": "python"}}
    )
...
@pytest.mark.asyncio
async def test_code_node_no_code_provided_fallback_to_content(
    code_node_applet, mock_subprocess_exec
):
    ...
    message = AppletMessage(
        content={"code": code, "value": 10}, metadata={"node_data": {"language": "python"}}
    )
...
async def test_code_node_stdout_truncated(code_node_applet):  # Removed mock_subprocess_exec
    ...
    with patch(
        "apps.orchestrator.main._read_stream_limited", new_callable=AsyncMock
    ) as mock_read_stream_limited:
        ...
        message = AppletMessage(
            ...
            metadata={
                "node_data": {
                    "language": "python",
                    "code": code,
                    "max_output_bytes": max_output_configured,
                }
            },
        )
