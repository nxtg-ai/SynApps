...
async def test_artist_applet_stability_success():
    with patch.dict(os.environ, {"STABILITY_API_KEY": "test_key"}):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"artifacts": [{"base64": "fake_image_data"}]}

        with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
            mock_post.return_value = mock_response

            message = AppletMessage(
                content="Test prompt", context={"image_generator": "stability"}, metadata={}
            )
            response = await applet.on_message(message)
...
async def test_artist_applet_openai_success():
    with patch.dict(os.environ, {"OPENAI_API_KEY": "test_key"}):
        ...
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"data": [{"b64_json": "fake_openai_image"}]}

        with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
            mock_post.return_value = mock_response

            message = AppletMessage(
                content="Test prompt", context={"image_generator": "openai"}, metadata={}
            )
            response = await applet.on_message(message)
...
async def test_memory_applet_store_retrieve():
    ...
    retrieve_message = AppletMessage(
        content={"operation": "retrieve", "key": "my_key"}, context={}, metadata={}
    )
...
async def test_memory_applet_retrieve_by_tags():
    ...
    # Retrieve by tag1
    retrieve_message = AppletMessage(
        content={"operation": "retrieve", "tags": ["tag1"]}, context={}, metadata={}
    )
...
async def test_artist_applet_input_variations():
    with patch.dict(os.environ, {"STABILITY_API_KEY": "test_key"}):
        applet = ArtistApplet()
        with patch(
            "apps.applets.artist.applet.ArtistApplet._call_stability_api", new_callable=AsyncMock
        ) as mock_stability:
            mock_stability.return_value = "img"

            # Test dict with 'text'
            await applet.on_message(
                AppletMessage(content={"text": "some text"}, context={}, metadata={})
            )
...
async def test_memory_applet_clear():
    ...
    await applet.on_message(
        AppletMessage(
            content={"operation": "store", "key": "k1", "data": "v1"}, context={}, metadata={}
        )
    )
    await applet.on_message(AppletMessage(content={"operation": "clear"}, context={}, metadata={}))

    response = await applet.on_message(
        AppletMessage(content={"operation": "retrieve", "key": "k1"}, context={}, metadata={})
    )
...
async def test_memory_applet_delete():
    ...
    # Delete existing
    response = await applet.on_message(
        AppletMessage(content={"operation": "delete", "key": "k1"}, context={}, metadata={})
    )
...
async def test_artist_applet_variations():
    with patch.dict(os.environ, {"STABILITY_API_KEY": "test_key", "OPENAI_API_KEY": "test_key"}):
        ...
        with patch(
            ...
            with patch(
                "apps.applets.artist.applet.ArtistApplet._call_openai_api", new_callable=AsyncMock
            ) as mock_openai:
                mock_openai.return_value = "openai_img"

                response = await applet.on_message(
                    AppletMessage(
                        content="test", context={"image_generator": "stability"}, metadata={}
                    )
                )
        ...
        with patch(
            ...
            response = await applet.on_message(
                AppletMessage(content="test", context={"image_generator": "openai"}, metadata={})
            )
...
async def test_writer_applet_with_system_prompt():
    with patch.dict(os.environ, {"OPENAI_API_KEY": "test_key"}):
        ...
        with patch("httpx.AsyncClient.post", new_callable=AsyncMock) as mock_post:
            ...
            message = AppletMessage(
                content="test", context={"system_prompt": "custom prompt"}, metadata={}
            )
...
async def test_artist_applet_style_and_errors():
    with patch.dict(os.environ, {"STABILITY_API_KEY": "test_key", "OPENAI_API_KEY": "test_key"}):
        ...
        with patch(
            ...
            mock_stability.return_value = "img"
            await applet.on_message(
                AppletMessage(content="test", context={"style": "cyberpunk"}, metadata={})
            )
...
async def test_memory_applet_not_found():
    applet = MemoryApplet()
    retrieve_message = AppletMessage(
        content={"operation": "retrieve", "key": "non_existent"}, context={}, metadata={}
    )
    response = await applet.on_message(retrieve_message)
    assert response.content["status"] == "not_found"
